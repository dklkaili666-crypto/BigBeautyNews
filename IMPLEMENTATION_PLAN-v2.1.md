...
| T21-002 全量回归、范围审查与重新发布 | FR-023；FR-020；NFR-013；NFR-014 | 运行定向和全量检查；审查差异只涉及翻译纠偏和对应测试；更新 v2.0/v2.1 计划状态，将实现、批准文档和 CR 提交并推送到现有 `master`。 | T21-001 | 16 项定向测试、74 项全量测试、Ruff、Mypy、`compileall`、`git diff --check` 全部通过；实现提交 `583f481`、`535528d`、`067c70a` 已推送；无范围外实现改动。 | Completed |
| T21-003 Flash 真实端到端验收 | FR-022；FR-023；NFR-013；NFR-014 | 使用现有 `daily.yml`，以 `trigger_source=manual`、`force_push=false` 触发一次真实运行并等待完成；检查四次 LLM 阶段、5+5输出、Schema、Server酱和数据提交。 | T21-002；当天尚未成功推送 | 前两次真实运行按安全规则暴露纠偏措辞不足且未推送；[run 32640356597](https://github.com/dklkaili666-crypto/BigBeautyNews/actions/runs/32640356597) 约61秒成功，Flash AI/政经各5条，Server酱 HTTP 200/code 0且仅一次，四个状态布尔值全为 true。 | Completed |
| T21-004 逐项验收与交付记录 | FR-018～FR-023；NFR-013～NFR-015 | 新增 `TRACEABILITY-v2.1.md`，更新 `CHANGELOG.md`、`PRD.md`、v2.0/v2.1计划状态；记录测试、提交、workflow URL和安全运行证据，完成孤儿任务/改动检查后提交并推送。 | T21-003 | `TRACEABILITY-v2.1.md` 逐条覆盖全部需求；根 PRD 与 Changelog 已更新；无孤儿任务或实现改动。 | Completed |
