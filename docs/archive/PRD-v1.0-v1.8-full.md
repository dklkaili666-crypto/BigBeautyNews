...
> 当前批准基线：v1.8 | 状态：Approved | 日期：2026-07-13
>
> 批准人：项目所有者 | 批准证据：用户于 2026-07-13 在当前任务中明确回复“批准 PRD v1.8”
>
...
## 10. v1.8 可靠性与可维护性改进（Draft）

### CR-001：处理审查建议 2–7，保留现有 Pages 状态

- **请求变更**：修复政经词形与美国缩写漏报；新增无副作用 CI 和生产前测试门槛；升级 GitHub Actions；锁定部署依赖；整理当前态 PRD；在不改变行为的前提下小步拆分主流水线。
- **原因**：2026-07-13 全项目审查发现政经规则存在可复现漏报，生产工作流缺少测试门槛，Actions 出现 Node.js 20 弃用警告，依赖不可复现，PRD 存在历史与当前状态混杂，`run_pipeline()` 已增长至约 460 行。
- **影响需求**：新增 FR-014～FR-016、NFR-008～NFR-011；修正文档中的 FR-012 真实状态。
- **不在范围内**：不关闭、不删除、不迁移 GitHub Pages；不改变仓库 Public 状态；不修改 Touyanrili；不增加新闻源、推送渠道或 LLM 调用；不改变 5+5 内容、时间、幂等和外部 JSON 契约。
- **完成工作影响**：v1.7 功能继续作为行为基线；FR-012 记录为用户接受的已知例外，而不是本轮待修功能。
- **推荐方案**：采用下列最小规则补丁、独立 CI、精确约束文件、文档归档和阶段函数拆分。
- **替代方案**：引入 NLP 词形库、全面重写流水线或更换部署平台；均因复杂度和范围过大而不采用。
- **拟议 PRD 版本**：v1.8。
- **状态**：Approved。

### 10.1 目标与成功指标

1. 常见政经复数词形和 `US` 标题不再被规则误杀，同时不把英文代词 `us` 误识别为美国；
2. 每次 push/PR 都有无外部副作用的自动测试，生产日报在调用 LLM 和 Server酱前通过快速测试门槛；
3. GitHub Actions 不再使用产生 Node.js 20 弃用警告的 action 大版本；
4. GitHub Actions 使用经过验证的精确依赖版本，避免无代码变化时解析到不同依赖；
5. 根 PRD 只描述当前批准状态，历史长文有独立归档，验收记录准确反映 Pages 已知例外；
6. 主流水线按阶段拆分但用户行为、数据契约、调用次数和推送结果不变；
7. 全量回归和新增测试全部通过。

### 10.2 功能需求

#### FR-014：完善政经词形与美国缩写识别

- **要求**：在现有轻量规则中支持 `sanction(s)`、`tariff(s)`、`election(s)`、`missile(s)`、`regulation(s)` 等明确词形；可靠识别标题或标准新闻写法中的 `US` / `U.S.`，但不得把普通英文代词 `us` 当成美国实体。
- **验收标准**：
  - `U.S. imposes new sanctions on China` 被保留；
  - `US raises tariffs on Chinese imports` 被保留；
  - `European elections reshape economic policy` 被保留；
  - `The policy tells us how families save` 不得仅因 `us` 被标记为美国事件；
  - 既有政经正反例与 AI/政经主板块分类测试继续通过；
  - 不新增 NLP 或词形处理第三方依赖。

#### FR-015：增加无副作用 CI 与生产前测试门槛

- **要求**：新增独立 GitHub Actions CI，在 push 和 pull request 上运行；CI 不读取生产 Secrets、不抓取实时新闻、不调用 LLM、不发送 Server酱。每日生产 workflow 在 `Run BigBeautyNews` 前执行本地快速测试，失败时不得进入抓取、LLM、持久化或推送。
- **验收标准**：
  - CI 使用 Python 3.12，至少执行 `compileall`、Pytest、Ruff 和 Mypy；
  - CI workflow 对 push 与 pull request 生效；
  - CI 文件不引用 `SERVERCHAN_SENDKEY`、`LLM_API_KEY` 或其他生产 Secret；
  - `daily.yml` 的测试步骤位于生产运行步骤之前，并受与正式运行一致的 `should_run` 条件约束；
  - 测试失败路径通过 workflow 静态测试证明不会执行 `python src/main.py`；
  - 现有主/兜底、手动触发、`push_test` 和幂等行为保持不变。

#### FR-016：升级 GitHub Actions 运行时

- **要求**：将 `actions/checkout` 和 `actions/setup-python` 升级到审查时确认的当前稳定大版本，消除真实运行中的 Node.js 20 弃用警告；不重新引入 Pages actions。
- **验收标准**：
  - CI 和 `daily.yml` 使用一致的 action 大版本；
  - workflow 测试固定批准版本并拒绝旧版本；
  - `daily.yml` 仍只保留 `contents: write` 权限，不出现 Pages/id-token 权限；
  - 至少有一次 GitHub Actions CI 运行成功作为远程证据。

### 10.3 非功能需求

#### NFR-008：依赖安装可复现

- **要求**：保留可读的直接依赖清单，新增部署/CI 使用的精确约束文件；开发检查工具也必须显式声明，不依赖执行机器偶然预装。
- **验收标准**：
  - 所有生产直接依赖和 CI 检查工具均有精确版本；
  - 本地与 GitHub Actions 使用同一约束文件安装；
  - 在干净环境中可完成安装、导入和全量测试；
  - 不进行与本轮无关的依赖升级。

#### NFR-009：PRD 只表达当前状态

- **要求**：将 v1.7 长文保存为历史归档；根 `PRD.md` 重写为简洁的 v1.8 当前态文档，保留稳定需求编号、当前架构、范围、风险、已知例外和版本记录，不再依赖“后文覆盖前文”的解释。
- **验收标准**：
  - 历史 v1.7 文档可追溯且内容不丢失；
  - 根 PRD 不再出现“v0.6 下一步开发目标”、旧 Pages 已停止等与真实状态冲突的表述；
  - Pages 明确记录为用户选择保留的公开服务，raw JSON 继续公开供投研日历使用；
  - TRACEABILITY 将原 FR-012 修正为用户接受的例外，不再错误宣称 Pages 已关闭；
  - 根 PRD 的每项当前需求都有可验证验收标准。

#### NFR-010：小步拆分主流水线

- **要求**：只拆分现有阶段职责，不引入框架、插件系统、通用工作流引擎或为未来需求预留抽象。`run_pipeline()` 保留顶层编排，抓取、候选准备、排序翻译、持久化推送等阶段移动到命名明确、可独立测试的函数。
- **验收标准**：
  - `run_pipeline()` 主要表达阶段顺序和失败返回，不再内嵌全部实现细节；
  - 每个新增函数只承担一个可描述职责；
  - 不改变公开 CLI、环境变量、输出路径、schema、调度和 Server酱调用；
  - 不新增无实际调用者的抽象、类或配置；
  - 相关阶段有直接单元测试或由既有端到端测试覆盖。

#### NFR-011：行为与成本回归保护

- **验收标准**：
  - AI 5 + 政经 5、单次 Server酱 POST、7:45/8:15、手动触发、幂等和投研日历五字段 AI JSON 均保持不变；
  - 正常 LLM 调用仍为 AI 排序/翻译各 1 次、政经排序/翻译各 1 次；
  - 既有 58 项测试和本轮新增测试全部通过；
  - Ruff、Mypy、compileall 和 `git diff --check` 通过；
  - 不修改 GitHub Pages 设置或 Touyanrili 项目。

### 10.4 风险与对策

| 风险 | 对策 |
|---|---|
| `US` 规则引入代词误报 | 保留原文大小写用于 `US` 判断，并加入明确负例 |
| 锁定依赖后漏掉间接依赖 | 以干净环境安装和全量测试验证 constraints |
| CI 增加 Actions 时间 | 测试无网络/LLM，当前全套约 1–2 秒，启用 pip 缓存 |
| 重构改变运行语义 | 先补边界测试，再逐阶段提取；每步运行相关测试与全量回归 |
| PRD 整理丢失历史 | 先保存完整 v1.7 归档，再重写根文档并做需求编号核对 |

### 10.5 已知例外与范围确认

- GitHub Pages 当前仍为 `public/built` 且公开 URL 可访问；用户明确选择本轮不处理。
- 该状态不影响投研日历 raw JSON，但意味着本地网页内容仍有额外公开入口。
- v1.8 不得调用 GitHub API 修改 Pages 设置，也不得把仓库改为 Private。

### 10.6 PRD 完整性检查

- 审查建议 2–7 均由 FR-014～FR-016、NFR-008～NFR-011 覆盖。
- 审查建议 1 明确排除并记录为用户接受的已知例外。
- 每项新增需求都有可观察验收标准。
- 不改变 v1.7 用户功能、成本边界或外部契约。
- 未决产品决策：无。
...
| v1.8 | 2026-07-13 | 已批准：修复政经词形漏报；新增 CI 与生产测试门槛；升级 Actions；锁定依赖；整理当前态 PRD；小步拆分主流水线；Pages 保持现状 |
